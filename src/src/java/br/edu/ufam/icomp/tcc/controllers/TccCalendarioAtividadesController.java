 ...
 */
@Resource
public class TccCalendarioAtividadesController {
    ...
    private TccAtividadeDAO tccatividadeDAO;
    private PeriodoLetivoDAO periodoletivoDAO;
    private TccTccDAO tcctccDAO;

    public TccCalendarioAtividadesController(ServletContext context, TccAtividadeDAO tccatividadeDAO, SessionData session, CursoDAO cursoDAO, AlunoDAO alunoDAO,
    ...
    */
    ...
    @Get("/tcc/calendarioAtividades")
    public Download tccCalendarioAtividades() {   

        PeriodoLetivo periodo = this.periodoletivoDAO.findLetivoAtivo();

        
        List<TccAtividade> tccatividades = this.tccatividadeDAO.findByPeriodo(periodo.getId());
        //deixei tcctccDAO.findAll pq não tenho o tcctccDAO.findByPeriodo.
        
       if (tccatividades.isEmpty()) {
            this.validator.add(new ValidationMessage("Desculpe! Não foi registrado nenhuma atividade de tcc para o periodo " + periodo.toString(), "."));
        }
        ...
        try {
            InputStream resourceAsStream = getClass().getResourceAsStream("/br/edu/ufam/icomp/tcc/jasper/reportatividadestcc.jrxml");
           ...
           // exemplo: nome_do_aluno_de_usuario / tcctcc_titulo / tccnotas_nota1 / tccnotas_nota2 / tccnotas_nota3
            ...
            return new InputStreamDownload(inputStream, "application/pdf", "CalendarioAtividadesTcc.pdf");
        } catch (JRException ex) {
            Logger.getLogger(TccCalendarioAtividadesController.class.getName()).log(Level.SEVERE, null, ex);
        }
