...
<html>
    <head>
        ...
        <title>Listando Coordenadores de Curso</title>
        ...
        <script language="JavaScript" type="text/javascript">
            function radioHab(variavel) {
                var table = document.getElementById('formCoordCurso');
                var Radio = null;
                ...
                if(Radio.checked){
                    switch (variavel){
                        case 'edit':
                            document.location.href = "${pageContext.request.contextPath}/coordCurso/" + Radio.value + "/edit";                          
                            return;
                        case 'remove':    
                            decisao = confirm("Deseja realmente remover o coordenador de curso?");
                            if (decisao){
                                document.location.href = "${pageContext.request.contextPath}/coordCurso/" + Radio.value + "/remove";                                                              
                            } else {
                                alert ("Nenhum coordenador de curso foi removido");
                            }
                            return;
                        case 'show':
                            document.location.href="${pageContext.request.contextPath}/coordCurso/"+Radio.value + "/show";
                            return;
                        }
                    } else {
                        for(var i = 0; i < Radio.length; i++) {
                            if(Radio[i].checked) {                    
                                switch (variavel){
                                    case 'edit':
                                        document.location.href="${pageContext.request.contextPath}/coordCurso/" + Radio[i].value + "/edit";                          
                                        return;
                                    case 'remove':
                                        decisao = confirm("Deseja realmente remover o coordenador de curso?");
                                        if (decisao){
                                            document.location.href="${pageContext.request.contextPath}/coordCurso/"+Radio[i].value + "/remove";                                                              
                                        } else {
                                            alert ("Nenhum coordenador de curso foi removido");
                                        }                            
                                        return;
                                    case 'show':
                                        document.location.href="${pageContext.request.contextPath}/coordCurso/" + Radio[i].value + "/show";                          
                                        return;
                                    }
                                }
                            }
                            alert('Você precisa selecionar um dos coordenadores de curso');
                        }
                    }
        </script>
    </head>
    <body id="dt_example">
        <div id="toolbar-box">
            <div class="m">
                            ...
                            <div class="icon">
                                <ul>
                                    <li class="button" id="toolbar-apply">
                                        <a href="${pageContext.request.contextPath}/coordCurso/create" id="new">
                ...
                <div class="pagetitle icon-48-article"><h2>Listagem de<br/>Coordenadores de Curso</h2></div>
            </div>
        </div>
        <div>
            <c:if test="${not empty success}">
                <div class="ui-corner-all alert-success" style="margin-top: 20px; padding: 0 .7em;">
                    <p>
                        ...
                        Coordenador de Curso ${success} com sucesso!
                    </p>
                </div>
                <br/>
            </c:if>
        ...
        <div id="demo">
            <form id="formCoordCurso">
                <table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="100%">
                    ...
                    <tbody>
                        <c:forEach items="${coordenadorCursoList}" var="coordCurso">
                            <tr>
                                <td class="center"><input type="radio" name="RadioGroup1" value="${coordCurso.id}"/></td>                            
                                <td class="left">${coordCurso.professor.usuario.nome}</td>
                                <td class="left">${coordCurso.curso}</td>
                            </tr>
                        </c:forEach>                    
                    </tbody>
                    <tfoot>
