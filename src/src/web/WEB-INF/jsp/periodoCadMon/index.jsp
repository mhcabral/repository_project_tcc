...
<head>
    <title>Períodos de Cadastro de Monitorias</title>            
    ...
    <script language="JavaScript" type="text/javascript">
        function radioHab(variavel) {
            ...
            if(Radio.checked){
                switch (variavel){
                    case 'edit':
                        document.location.href="${pageContext.request.contextPath}/periodos/"+Radio.value + "/edit";                          
                        return;
                    case 'remove':    
                        decisao = confirm("Deseja realmente remover o período?");
                        if (decisao){
                            document.location.href="${pageContext.request.contextPath}/periodos/"+Radio.value + "/remove";                                                              
                        } else {
                            alert ("Nenhum Período foi removido");
                        }                            
                        return;
                    case 'show':
                        document.location.href="${pageContext.request.contextPath}/periodos/"+Radio.value + "/show";                          
                        return;
                    }
                } else {
                    for(var i=0;i<Radio.length;i++) 
                    {
                        ...
                        {                    
                            switch (variavel){
                                case 'edit':
                                    document.location.href="${pageContext.request.contextPath}/periodos/"+Radio[i].value + "/edit";                          
                                    return;
                                case 'remove':    
                                    decisao = confirm("Deseja realmente remover o período?");
                                    if (decisao){
                                        document.location.href="${pageContext.request.contextPath}/periodos/"+Radio[i].value + "/remove";                                                              
                                    } else {
                                        alert ("Nenhum Período foi removido");
                                    }                            
                                    return;
                                case 'show':
                                    document.location.href="${pageContext.request.contextPath}/periodos/"+Radio[i].value + "/show";                          
                                    return;
                                }                    
                            }                
                        }
                    
                        alert('Você precisa selecionar um dos períodos');
                    }
                }
    </script>
</head>
<body id="dt_example">
    <div id="toolbar-box">
        <div class="m">
                        ...
                        <div class="icon">
                            <ul>
                                <li class="button" id="toolbar-apply">
                                    <a href="${pageContext.request.contextPath}/periodos/create" id="new">
            ...
            <div class="pagetitle icon-48-article"><h2>Listagem de Períodos:<br/>Cadastro de Monitoria</h2></div>
        </div>
    </div>
    <div>
    ...
    <div id="demo">        
        <form id="formPeriodo">
            <table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="100%">
                ...
                <tbody>                    
                    <c:forEach items="${periodoCadMonList}" var="periodo">
                        <tr>
                            <td class="center"><input type="radio" name="RadioGroup1" value="${periodo.id}"/></td>                            
                            <td class="center">${periodo.curso}</td>
