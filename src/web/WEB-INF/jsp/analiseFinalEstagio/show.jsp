            ...
            $(function(){
                $('#aprovar').click(function(e) {
                    ...
                    if (decisao){                        
                        ...
                        $("#formAnalise").attr('action', '${pageContext.request.contextPath}/analise/final/estagios/avaliacao').submit();
                    }
                });
            });
    ...
    <body>
        ...
        <c:if test="${not empty estagio.id}">
            ...
            <p>
                ...
                <a href="${pageContext.request.contextPath}/analise/final/estagios/download/${estagio.termoCompromisso}" target="_blank" >Download</a>
            </p>
            <p>
                <b>Seguro:</b>
                <a href="${pageContext.request.contextPath}/analise/final/estagios/download/${estagio.seguro}" target="_blank" >Download</a>
            </p>
            <c:if test="${estagio.convenio == false}">
                <p>
                    <b>Ficha de Cadastro da Empresa:</b>
                    <a href="${pageContext.request.contextPath}/analise/final/estagios/download/${estagio.fichaCadastroEmpresa}" target="_blank" >Download</a>
                </p>
            </c:if>
            <br/>
            <h4>Relatório Final</h4>
            <p>
                <c:if test="${not empty estagio.relatorioFinal}">
                    <a href="${pageContext.request.contextPath}/analise/final/estagios/download/${estagio.relatorioFinal.relatorioFinal}" target="_blank" >Download</a>
                </c:if>
            </p>
            <div>
                <c:if test="${empty estagio.relatorioFinal}">
                ...
                <table width="15%" style="border: solid 1px #8099B3">
                    ...
                    <c:forEach items="${freqAnalisadas}" var="frequencia">
                        <tr>
                            ...
                            <c:if test="${frequencia.status != 'VAZIA' && frequencia.status != 'SALVA'}">
                                <td width="5%" style="border: solid 1px #8099B3">
                                    ${frequencia.aprovacoes[0].nomeAprovador}
                                </td>
                                <td width="5%" style="border: solid 1px #8099B3">
                                    <c:if test="${frequencia.aprovacoes[0].aprovou == true}">
                                        Aprovado
                                    </c:if>
                                    <c:if test="${frequencia.aprovacoes[0].aprovou == false}">
                                        Rejeitado ou Pendente
                                    </c:if>
                                </td>
                                <td width="5%" style="border: solid 1px #8099B3">
                                    ${frequencia.aprovacoes[1].nomeAprovador}
                                </td>
                                <td width="5%" style="border: solid 1px #8099B3">
                                    <c:if test="${frequencia.aprovacoes[1].aprovou == true}">
                                        Aprovado
                                    </c:if>
                                    <c:if test="${frequencia.aprovacoes[1].aprovou == false}">
                                        Rejeitado ou Pendente
                                    </c:if>
                                </td>
                            </c:if>
