...
<head>
    <title>Listando Histórico das Solicitações</title>            
    ...
    <script language="JavaScript" type="text/javascript">
        function radioHab(variavel) {
            ...
            if(Radio.checked){
                switch (variavel){
                    case 'show':
                        document.location.href="${pageContext.request.contextPath}/analise/historico/"+Radio.value + "/show";                          
                        return;
                    }
                } else {
                    for(var i=0;i<Radio.length;i++) 
                    {
                        if(Radio[i].checked) 
                        {
                            switch (variavel){
                                case 'show':
                                    document.location.href="${pageContext.request.contextPath}/analise/historico/"+Radio[i].value + "/show";
                                    return;
                                }
                            }
                        }
                ...
                function adjustVagas() {                            
                    var peridoLetivo = $('#campo-periodoLetivo').val();                                                        
                    var curso = $('#campo-curso').val();
                    var atividade =$('#campo-atividade').val();
                    ...
                    window.location = "${pageContext.request.contextPath}/analise/historico/"+atividade+"/"+peridoLetivo+"/"+curso+"/"+aluno+"/search";
                }
    </script>   
    <style>
        .ui-combobox {
    ...
    <div id="container">
        <div id="toolbar-box">
            <div class="m">
                ...
                <div class="pagetitle icon-48-notice"><h2>Histórico das Solicitações</h2></div>
            </div>
        </div>
        <div>
        ...
        <div class="ui-widget">
            ...
            <select id="campo-aluno" name="ccampo-aluno">
                <option value="0">Todos</option>
            ...
            <select id="campo-atividade" name="campo-atividade">
                ...
                <c:forEach var="atividade" items="${atividadeList}">
                    <option value="${atividade.id}" <c:if test = "${atividade.id == idAtividade}"> selected="true"</c:if>>${atividade}</option>
                </c:forEach>
            </select>           
        </div>
    </p>
    ...
    <div id="demo">
        <form id="formSolicitacao">
            <table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="100%">
                ...
                <tbody>
                    <c:forEach items="${solicitacaoNaoAnalisavelList}" var="solicitacao">
                        <tr>
                            <td><input type="radio" name="RadioGroup1" value="${solicitacao.id}"/></td>
                            <td class="center">${solicitacao.periodo.codigo}</td>
                            ...
                            <td class="center"><a href="${pageContext.request.contextPath}/analise/historico/${solicitacao.id}/show"><img src="${pageContext.request.contextPath}/img/show.png"></img></a></td>
                        </tr>
                    </c:forEach>                    
                </tbody>
                <tfoot>
